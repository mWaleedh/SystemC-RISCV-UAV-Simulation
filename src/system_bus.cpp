...
SC_MODULE(system_bus) {
    ...
    void routeBus() {
        ...
        // Route bus to Interrupt Manager
        else if (addr == 0x10000010 || addr == 0x10000014) {
        ...
        cpu_data_bus_o.write(mem_data_bus_i.read() | gpio_data_bus_i.read());
    }

    SC_CTOR(system_bus) {
        SC_METHOD(routeBus);
        sensitive << cpu_read_en_i << cpu_write_en_i << cpu_addr_bus_i << cpu_data_bus_i << mem_data_bus_i << gpio_data_bus_i;
    }
};